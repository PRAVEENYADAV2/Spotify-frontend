...
export default function Navigation({
  ...
  return (
    <div
      ...
      style={{ height: "calc(100vh - 8px)", width: `${width}px` }}
    >
      ...
      >
        <div>
          <header className="flex items-center justify-between">
            ...
            >
              ...
              {isOpen && (
                <span className="font-bold text-[#a7a7a7] group-hover:text-white">Your Library</span>
              )}
              </button>
            </div>
            {isOpen && (
              <div className="flex gap-3">
                ...
                <button
                  className="hover:bg-[#414141] rounded-full p-3"
        ...
        <div
          className="w-full overflow-auto mt-3"
...
function NavList({ isOpen }: { isOpen: boolean }) {
  return (
    <ul className="flex flex-col gap-6" id="navList">
      <li>
        <a href="">
          <div className="flex gap-3">
            <svg
...
function Item({ type, isOpen }: { type: ItemType; isOpen: boolean }) {
  if (!isOpen) {
    return (
      <div
        className={`flex flex-col gap-2  rounded-lg cursor-pointer group ${
          isOpen ? "my-2 p-3 hover:bg-[#ababab1c] transition-all" : "mb-1"
        }`}
      >
